...
上周工作：
1、遥操辅助软件，增加交互提示功能，明确颜色变化触发条件；添加关闭点云功能的按键提示
2、伺服上位机软件改进​，实现配置信息本地化存储，保存用户习惯。封装数据库工具类，方便后续数据库读写，。
本周工作：
1、继续配合伺服上位机软件开发，根据需求合并功能和硬件测试；
2、三维重建加速，存一些素材；固定模式
3、发个版
...
上周工作：
1、伺服上位机：
	新增双串口通信功能，实现对扭矩传感器的数据读取；
	修复示波器运行期间主窗口串口数据抢占问题；
	优化在线更新机制，增加服务端地址修改与连接支持；
下周工作：
1、遥操辅助软件，增加交互提示功能，明确颜色变化触发条件；添加关闭点云功能的按键提示
2、继续配合伺服上位机软件开发，完成扭矩传感器相关测试
